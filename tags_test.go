...
func TestRender_TagsNextGet(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantName: "/tags/autoComplete/tags?tagPrefix=DB",
		},
		{
			...
			wantName: "/tags/autoComplete/values?expr=name%3Dk8s.test-cl1.kube_%2A_labels&expr=namespace%3Dweb&tag=pod&limit=10000",
		},
		{
			wantURL:  baseURL + "/tags/autoComplete/tags?tagPrefix=DB",
			wantName: "/tags/autoComplete/tags?tagPrefix=DB",
	...
	for n, tt := range tests {
		t.Run(fmt.Sprintf("[%d]", n), func(t *testing.T) {
			gotURL, gotName, err := m.TagsNextGet("tags", 0)
			if err != nil {
				...
				if gotName != tt.wantName+"&label=tags" {
					t.Errorf("Carbonapi.TagsNextGet(\"%s\", \"json\").name got\n'%s'\nbut want\n'%s'", "tags", gotName, tt.wantName+"&label=tags")
				}
			}
		})
	}
