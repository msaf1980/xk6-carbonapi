...
let THRESHOLD_403_PCNT_FIND = getenv.getFloat(extractEnvParams(K6_CARBONAPI_PARAMS, "THRESHOLD_403_PCNT_FIND"), 1.0) / 100.0
...
let THRESHOLD_403_PCNT_TAGS = getenv.getFloat(extractEnvParams(K6_CARBONAPI_PARAMS, "THRESHOLD_403_PCNT_TAGS"), 1.0) / 100.0
...
if (USERS_FIND > 0 && FIND != "") {
    ...
    thresholds['checks{status:forbidden, label:find}'] = [
        {
            threshold: `rate<${THRESHOLD_403_PCNT_FIND}`, // http 403 errors should be less than
            abortOnFail: true,
            delayAbortEval: '120s',
        },
    ]
...
if (USERS_TAGS > 0 && TAGS != "") {
    ...
    thresholds['checks{status:forbidden, label:tags}'] = [
        {
            threshold: `rate<${THRESHOLD_403_PCNT_TAGS}`, // http 403 errors should be less than
            abortOnFail: true,
            delayAbortEval: '120s',
        },
    ]
