...
func TestRender_RenderNextGetJSON(t *testing.T) {
	...
	for n, tt := range tests {
		t.Run(fmt.Sprintf("[%d] %s", n, tt.name), func(t *testing.T) {
			...
			if err != nil {
				t.Errorf("Carbonapi.RenderNextGet(\"%s\", \"json\") got error '%v'", tt.name, err)
			} else {
...
func TestRender_RenderNext_pb_v3(t *testing.T) {
	...
	for n, tt := range tests {
		t.Run(fmt.Sprintf("[%d] %s", n, tt.name), func(t *testing.T) {
			...
			if err != nil {
				t.Errorf("Carbonapi.RenderNextPb_v3(\"%s\") got error '%v'", tt.name, err)
			} else {
				if gotURL != tt.wantURL {
				...
				if gotFetchReq, err := m.DecodeRenderReqPb_v3(reqBody); err == nil {
					...
					t.Errorf("Carbonapi.RenderNextPb_v3(\"%s\").reqBody error = %v", tt.name, err)
				}
